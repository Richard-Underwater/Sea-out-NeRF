...
from internal.train_utils import sliced_wasserstein_loss, depth_ranking_loss
    ...
    with logging_redirect_tqdm():
        ...
        for step in tbar:
            ...
            with accelerator.autocast():
                renderings, ray_history,renderings_entire,trans,weights_all= model(
                    True,
                    batch,
            ...
            # supervised by data
            data_loss, stats = train_utils.compute_data_loss(batch, renderings_entire, config)
            losses['data'] = data_loss
            losses['bg_uw'] = 0* ((renderings_entire[0]['rgbs_uw']-med_bg)**2).mean()
            losses['depth'] = 0.00005*depth_ranking_loss(renderings[-1]['depth'],batch['depth'])
            ...
            if config.train_render_every > 0 and step % config.train_render_every == 0:
                ...
                rendering,rendering_entire= models.render_image(model, accelerator,
                                                test_batch, False,
                                                train_frac, config)

                # move to numpy
                rendering = tree_map(lambda x: x.detach().cpu().numpy(), rendering)
                rendering_entire = tree_map(lambda x: x.detach().cpu().numpy(), rendering_entire)
                # rendering_J = tree_map(lambda x: x.detach().cpu().numpy(), rendering_J)
                ...
                if accelerator.is_main_process:
                    ...
                    vis_suite_entire = vis.visualize_suite_all(rendering_entire,test_batch)
                    ...
                    for k, v in vis_suite_entire.items():
                        summary_writer.add_image('test_entire_output_' + k, tb_process_fn(v), step)
