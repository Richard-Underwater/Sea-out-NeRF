...
def visualize_suite_all(rendering, batch):
    """A wrapper around other visualizations for easy integration."""

    depth_curve_fn = lambda x: -np.log(x + np.finfo(np.float32).eps)

    rgb = rendering['rgb']
    rgb_bg_med = rendering['rgb_bg_med']
    rgb_med = rendering['rgb_med']
    rgb_obj = rendering['rgb_obj']
    acc = rendering['acc']
    ...
    # acc = np.where(np.isnan(distance_mean), np.zeros_like(acc), np.zeros_like(acc))
