import accelerate
import gin
from internal import coord, render_single
...
class Model(nn.Module):
    ...
    def __init__(self, config=None, **kwargs):
        ...
        density_tensor_direct=torch.nn.Parameter(torch.Tensor([[0.5,0.5,0.5]]))
        nn.init.uniform_(density_tensor_direct)
        self.density_direct = density_tensor_direct
        density_tensor_forward = torch.nn.Parameter(torch.Tensor([[0.5, 0.5, 0.5]]))
        nn.init.uniform_(density_tensor_forward)
        self.density_forward = density_tensor_forward
        density_tensor_backscatter = torch.nn.Parameter(torch.Tensor([[0.5, 0.5, 0.5]]))
        nn.init.uniform_(density_tensor_backscatter)
        self.density_backscatter = density_tensor_backscatter
        rgb_tensor = torch.nn.Parameter(torch.Tensor([[1.0,1.0,1.0]]))
        nn.init.uniform_(rgb_tensor)
        self.rgb = rgb_tensor
        density_num = torch.nn.Parameter(torch.Tensor([[0.5]]))
        nn.init.uniform_(density_num)
        self.density_num = density_num
        density_num_2 = torch.nn.Parameter(torch.Tensor([[0.5]]))
        nn.init.uniform_(density_num_2)
        self.density_num_2 = density_num_2
        self.water_mlp = WaterMLP(num_glo_features=self.num_glo_features,
                                num_glo_embeddings=self.num_glo_embeddings)
        if self.single_mlp:
    ...
    """
        ...
        for i_level in range(self.num_levels):
            ...
            weights,_,trans = render.compute_alpha_weights(
                ray_results['density'],
                tdist,
                batch['directions'],
                opaque_background=self.opaque_background,
            )
            weights_s=render.compute_alpha_weights(
                ray_results['density'],
                sdist,
                batch['directions'],
                opaque_background=self.opaque_background,
        ...
        num_samples=12
        weights_new = torch.ones_like(batch['near'])
        sdist_aver = torch.cat([
            torch.full_like(batch['near'], init_s_near),
            torch.full_like(batch['far'], init_s_far)
        ], dim=-1)
        if self.anneal_slope > 0:
        ...
        logits_resample = torch.where(
            sdist_aver[..., 1:] > sdist_aver[..., :-1],
            anneal * torch.log(weights_new + self.resample_padding),
            torch.full_like(sdist_aver[..., :-1], -torch.inf))
        sdist_aver = stepfun.sample_intervals(
            rand,
            sdist_aver,
            logits_resample,
            num_samples,
            ...
            domain=(init_s_near, init_s_far))
        if self.stop_level_grad:
            sdist_aver = sdist_aver.detach()
        tdist_aver=s_to_t(sdist_aver)
        tdist_all=torch.sort(torch.cat([tdist, tdist_aver], dim=-1), dim=-1).values
        # tdist_all = tdist
        means_uwob, stds_uwob, ts_uwob = render.cast_rays(
            tdist_all,
            batch['origins'],
            batch['directions'],
            batch['cam_dirs'],
        ...
        #     exposure=batch.get('exposure_values'),
        # )
        density_direct = self.density_direct.expand_as(ray_results_all['density'].unsqueeze(-1).repeat_interleave(repeats=3,dim=-1))
        # density_direct_after = self.density_direct*self.density_num.expand_as(ray_results_all['density'].unsqueeze(-1).repeat_interleave(repeats=3,dim=-1))
        # density_direct_after_2 = self.density_direct * self.density_num_2.expand_as(
        #     ray_results_all['density'].unsqueeze(-1).repeat_interleave(repeats=3, dim=-1))
        density_forward = self.density_forward.expand_as(ray_results_all['density'].unsqueeze(-1).repeat_interleave(repeats=3,dim=-1))
        density_backscatter = self.density_backscatter.expand_as(ray_results_all['density'].unsqueeze(-1).repeat_interleave(repeats=3,dim=-1))
        rgbs_backscatter = self.rgb.expand_as(ray_results_all['rgb'])
        density_oball_direct = density_direct+ray_results_all['density'].unsqueeze(-1).repeat_interleave(repeats=3,dim=-1)
        # density_oball_direct_after = density_direct_after + ray_results_all['density'].unsqueeze(-1).repeat_interleave(repeats=3,
        #                                                                                                 dim=-1)
        # density_oball_direct_after_2 = density_direct_after_2 + ray_results_all['density'].unsqueeze(-1).repeat_interleave(repeats=3,
        #                                                                                                 dim=-1)
        density_oball_forward = density_forward + ray_results_all['density'].unsqueeze(-1).repeat_interleave(repeats=3,dim=-1)
        density_oball_backscatter = density_backscatter + ray_results_all['density'].unsqueeze(-1).repeat_interleave(repeats=3,
                                                                                                           dim=-1)

        weights_all = render.compute_alpha_weights(
            ray_results_all['density'],
            tdist_all,
            batch['directions'],
            opaque_background=self.opaque_background,
        )[0]
        weights_direct = render_single.compute_alpha_weights_oball(
            ray_results_all['density'],
            density_oball_direct,
            tdist_all,
            batch['directions'],
            opaque_background=self.opaque_background,
        )[0]
        weights_backscatter = render_single.compute_alpha_weights_oball(
            density_direct,
            density_oball_direct,
            tdist_all,
            batch['directions'],
        ...
        weights_forward = render_single.compute_alpha_weights_oball(
            density_direct,
            density_oball_direct,
            tdist_all,
            batch['directions'],
        ...
        weights_zero,_,trans_zero = render.compute_alpha_weights(
            ray_results_all['density'],
            tdist_all,
            batch['directions'],
            opaque_background=self.opaque_background,
        )
        rendering_all=render_single.volumetric_rendering_oball(
            ray_results_all['rgb'],
            rgbs_backscatter,
            weights_all,
            weights_direct,
        ...
        if compute_extras:
            n = self.config.vis_num_rays
            rendering_all['ray_sdist'] = sdist.reshape([-1, sdist.shape[-1]])[:n, :]
            rendering_all['ray_weights'] = (
                weights_new.reshape([-1, weights_new.shape[-1]])[:n, :])
            rgb = ray_results['rgb']
            rendering_all['ray_rgbs'] = (rgb.reshape((-1,) + rgb.shape[-2:]))[:n, :, :]
        renderings_entire=[]
        renderings_entire.append(rendering_all)
        if compute_extras:
        ...
        if compute_extras:
            weights = [r['ray_weights'] for r in renderings_entire]
            rgbs = [r['ray_rgbs'] for r in renderings_entire]
            final_rgb = torch.sum(rgbs[-1] * weights[-1][..., None], dim=-2)
            avg_rgbs = [
                torch.broadcast_to(final_rgb[:, None, :], r.shape) for r in rgbs[:-1]
            ]
            for i in range(len(avg_rgbs)):
                renderings_entire[i]['ray_rgbs'] = avg_rgbs[i]


        return renderings, ray_history,renderings_entire,trans_zero,weights_all
...
class MLP_3(nn.Module):
    ...
    def __init__(self, **kwargs):
        ...
        self.density_layer = nn.Sequential(nn.Linear(last_dim, 64),
                                           nn.ReLU(),
                                           nn.Linear(64,
                                                     3 if self.disable_rgb else self.bottleneck_width))  # Hardcoded to a single channel.
        last_dim = 1 if self.disable_rgb and not self.enable_pred_normals else self.bottleneck_width
        if self.enable_pred_normals:
            self.normal_layer = nn.Linear(last_dim, 3)
    ...
    def predict_density(self, means, stds, rand=False, no_warp=False):
        ...
        if self.scale_featurization:
            ...
            features = torch.cat([features, featurized_w], dim=-1)
        x = self.density_layer(features)
        raw_density = x[...,:3]  # Hardcoded to a single channel.
        # Add noise to regularize the density predictions if needed.
        if rand and (self.density_noise > 0):
            raw_density += self.density_noise * torch.randn_like(raw_density)
        return raw_density, x, means.mean(dim=-2)
  ...
  """
    ...
    for i_chunk, idx0 in enumerate(idx0s):
        ...
        with accelerator.autocast():
            chunk_renderings, ray_history,chunk_renderings_entire,_,_= model(rand,
                                                  chunk_batch,
        ...
        chunk_renderings_entire=tree_map(gather, chunk_renderings_entire)

        chunk_rendering = chunk_renderings[-1]
        chunk_rendering_entire=chunk_renderings_entire[-1]

        for k in chunk_renderings[0]:
            if k.startswith('ray_'):
                chunk_rendering[k] = [r[k] for r in chunk_renderings]
        for k in chunk_renderings_entire[0]:
            if k.startswith('ray_'):
                chunk_rendering_entire[k] = [r[k] for r in chunk_renderings_entire]

        if return_weights:
            chunk_rendering['weights'] = gather(ray_history[-1]['weights'])
            chunk_rendering['coord'] = gather(ray_history[-1]['coord'])
        if return_weights:
            chunk_rendering_entire['weights'] = gather(ray_history[-1]['weights'])
            chunk_rendering_entire['coord'] = gather(ray_history[-1]['coord'])
        chunks.append(chunk_rendering)
        chunks_entire.append(chunk_rendering_entire)
    ...
    for k in chunks_entire[0].keys():
        if isinstance(chunks_entire[0][k], list):
            rendering_entire[k] = []
            for i in range(len(chunks_entire[0][k])):
                rendering_entire[k].append(torch.cat([item[k][i] for item in chunks_entire]))
        else:
            rendering_entire[k] = torch.cat([item[k] for item in chunks_entire])
    ...
    for k, z in rendering_entire.items():
        if not k.startswith('ray_'):
            # Reshape 2D buffers into original image shape.
            rendering_entire[k] = z.reshape((height, width) + z.shape[1:])
    ...
    keys_entire = [k for k in rendering_entire if k.startswith('ray_')]
    if keys_entire:
        num_rays_entire = rendering_entire[keys_entire[0]][0].shape[0]
        ray_idx_entire = torch.randperm(num_rays_entire)
        ray_idx_entire = ray_idx_entire[:config.vis_num_rays]
        for k in keys_entire:
            rendering_entire[k] = [r[ray_idx_entire] for r in rendering_entire[k]]
    model.train()
    return rendering,rendering_entire
